...
export function startRoundTimer(timer: Clock , toDo, every?: boolean): void {
    setInterval(()=> {
        ...
        let conditionForSeconds: boolean = !timer.seconds ? true : secondsNow === timer.seconds;
        if(secondsNow == 0 && every) {
            if(60 % timer.seconds == 0) conditionForSeconds = conditionForSeconds = 60 % timer.seconds == 0
        ...
        let conditionForMinutes: boolean = !timer.minutes ? true : minutesNow === timer.minutes;
        if(minutesNow == 0 && every) {
            if(60 % timer.minutes == 0) conditionForMinutes = conditionForMinutes = 60 % timer.minutes == 0
        ...
        let conditionForHours: boolean = !timer.hours ? true : hoursNow === timer.hours;
        if(hoursNow == 0 && every) {
            if(24 % timer.hours == 0) conditionForHours = conditionForHours = 24 % timer.hours == 0
